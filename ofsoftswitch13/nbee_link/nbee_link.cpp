...
extern "C" int nblink_packet_parse(struct ofpbuf * pktin,  struct ofl_match * pktout, struct protocols_std * pkt_proto)
{
    ...
    cout<<PDMLReader->GetCurrentPacket(&curr_packet) <<endl;
        ...
        /* Getting first field of the protocol  */
            ...
            else if (protocol_Name.compare("ipv6") == 0 && pkt_proto->ipv6 == NULL)
            {
                ...
                if (PDMLReader->GetPDMLField(proto->Name, (char*) "HBH", proto->FirstField, &field) == nbSUCCESS){ 
                    cout<<offsetnon0<<"&&&&&"<<endl;
                    if(offsetnon0){
                        cout<<"+++++++++++++++++++++++++++++++++++++"<<endl;
                        ext_hdrs = (uint16_t*) EH_field->value;
                        *ext_hdrs ^=  OFPIEH_UNDHF;
                        *ext_hdrs = *ext_hdrs;
                        break;
                    } else                    
                ...
                if(PDMLReader->GetPDMLField(proto->Name, (char*) "FH", proto->FirstField, &field) == nbSUCCESS){
                    ...
                    if(PDMLReader->GetPDMLField(proto->Name, (char*) "fragment offset", proto->FirstField, &field) == nbSUCCESS)
                        if(!strcmp(field->Value,"0001")){
                            offset=true;
                            // cout<<ext_hdrs<<endl;
                        ...
                        PDMLReader->GetPDMLField(proto->Name, (char*) "identification", proto->FirstField, &field);
                        nblink_extract_proto_fields(pktin, field, pktout, OXM_OF_FRAG_ID);
                ...
                //it was added to check hop by hop fragment not compleate               
                if (PDMLReader->GetPDMLField(proto->Name, (char*) "HBH", proto->FirstField, &field) == nbSUCCESS){ 
                    cout<<offsetnon0<<"&&&&&"<<endl;
                    if(offsetnon0){
                        cout<<"+++++++++++++++++++++++++++++++++++++"<<endl;
                ...
                if (ip_proto){
                    if(!strcmp(ip_proto->Value,"3C")||!strcmp(ip_proto->Value,"00"))
                        protocol=true; 
                    nblink_extract_proto_fields(pktin, ip_proto, pktout, OXM_OF_IP_PROTO);  
                    cout<<ip_proto->Value<<" ip_proto"<<endl;
                }
                else 
                    protocol=true;           
            }
            ...
            if (protocol_Name.compare("icmp") == 0 && pkt_proto->icmp == NULL){
                ...
                PDMLReader->GetPDMLField(proto->Name, (char*) "type", proto->FirstField, &field) ;
                // cout<< ">>>> " << field->Value <<" <<icmp<<"<<endl;
                nblink_extract_proto_fields(pktin, field, pktout, OXM_OF_ICMPV6_TYPE);
                cout<< PDMLReader->GetPDMLField(proto->Name, (char*) "code", proto->FirstField, &field) <<endl;                
            ...
            if(protocol && offset){
                ext_hdrs = (uint16_t*) EH_field->value;
                *ext_hdrs ^=  OFPIEH_UNDIP;
                *ext_hdrs = *ext_hdrs;
            }

            if(!otherinfo && offset){
                ext_hdrs = (uint16_t*) EH_field->value;
                *ext_hdrs ^=  OFPIEH_UNDH;
                cout<<ext_hdrs<<"^^"<<endl;
                *ext_hdrs = *ext_hdrs;
            }            

            // cout<<endl<<otherinfo<< " -*********- "<<offset<<endl;
            // if(protocol && offsetnon0){
            //     ext_hdrs = (uint16_t*) EH_field->value;
            //     *ext_hdrs ^=  OFPIEH_UNDHF;
            //     cout<<ext_hdrs<<"^^"<<endl;
            //     *ext_hdrs = *ext_hdrs;
            //     cout<<protocol << offsetnon0<<endl;
            // }    

            // if( (!otherinfo && offset) || (protocol && offset)){
            //     ext_hdrs = (uint16_t*) EH_field->value;
            //     *ext_hdrs ^=  OFPIEH_AYMAN;
            //     *ext_hdrs = *ext_hdrs;
            //     // cout<<"======="<<endl;
            //     // cout<<ext_hdrs<<endl;
            // }
